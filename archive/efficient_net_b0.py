...
base = efn.EfficientNetB0(weights='imagenet', include_top=False, input_shape=(img_size, img_size, channels))
model = Sequential()
model.add(base)
...
def augment_image(img):
    ...
    # random crop
    crop_size = 380
    w_range = width - crop_size
    h_range = height - crop_size
    w = np.random.randint(low=0, high=w_range)
    h = np.random.randint(low=0, high=h_range)
    img = img[h:h+h_range, w:w-w_range]
