...
# sd_images = sorted(set(sd198_images + sd260_images))
sd_images = sorted(set(sd198_images))
...
for skin_class\
    ...
    start = time.time()
    try:
        os.mkdir(f'D:/Data/ISIC-2019/images_train_external_SD198/{skin_class}')
    except FileExistsError:
        pass
    subset = sd_data.loc[sd_data['class'] == skin_class].copy()
    for img_path in subset['path'].tolist():
        img_name = img_path.split('\\')[-1]
        shutil.copyfile(img_path, f'D:/Data/ISIC-2019/images_train_external_SD198/{skin_class}/{dataset}_{img_name}')
